...
const Library = () => {
    ...
    const deleteBook = async (id) => {
        await BookService.deleteBook(id).then((response) => {
            toast.success(response.data);
            getBooks()
        }).catch((err) => { console.log(err) })
        const afterDeletedBook = books.filter((b, i) => b.bid !== id)
        setBooks(afterDeletedBook)
    }
    useEffect(() => {
        getBooks();
    }, [])
                        ...
                        {books.map((book, i) => {
                            return (
                                ...
                                <tr className='text-center font-poppins'>
                                    <td>{i + 1}</td>
                                    <td className=''><img className='h-20 w-20' src={book.bookImage} alt="" /></td>
                                    <td>{book.bookName}</td>
                                    <td>{book.bookAuthorName}</td>
                                    <td>{book.bookShelfNo}</td>
                                    <td className='flex justify-evenly items-center h-20 w-full '>
                                        <img onClick={() => { navigate(`/viewBook/` + book.bid) }} src={viewIcon} className='p-1 rounded-md h-7  cursor-pointer w-7 bg-white ' alt="" />
                                        <img onClick={() => { navigate(`/editBook/` + book.bid) }} src={editIcon} className='p-1 rounded-md h-7  cursor-pointer w-7 bg-blue-200 ' alt="" />
                                        <img onClick={() => deleteBook(book.bid)} src={deleteIcon} className='p-1 rounded-md h-7  cursor-pointer w-7 bg-red-200' alt="" />
                                    </td>
                                </tr>
                            )
                        })}
